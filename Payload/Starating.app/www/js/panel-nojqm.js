define(['jquery', 'user', 'sidr', 'squad_database', 'rest', 'socialsharing'], function ($, user, sidr, squad_db, rest, socialsharing) {
    ...
    init: function(){
      ...
      //$(document).one("pagebeforecreate", function () {
        $.get('panel.html', function(data){ 
          ...
          if (userObj.linked_account_id) {
            if (userObj.profile_image) {
              if (userObj.linked_account_type == 2) { //fb  graph.facebook.com/10152380678997793/picture?type=large
                $('.sm-pp').attr('style', 'background-image: url("'+userObj.profile_image+'?type=large"); border: 1px solid #ffffff; background-size: 64px auto;');
                ...
                console.log(tmpImg);
                $('.sm-pp').attr('style', 'background-image: url("'+tmpImg+'"); border: 1px solid #ffffff; background-size: 64px auto;');
              }
            } else {
              console.log('no profile_image');
              if (userObj.linked_account_type == 2) { //fb  graph.facebook.com/10152380678997793/picture?type=large
                console.log('fb');
                $('.sm-pp').attr('style', 'background-image: url("http://graph.facebook.com/'+userObj.linked_account_id+'/picture?type=large"); border: 1px solid #ffffff; background-size: 64px auto;');
              } else if (userObj.linked_account_type == 1) {
          ...
          $('#sm-b-blog').on(clickEvent, function() {
            window.open('http://www.starating.net/blog', '_system'); return false;
          });

          $('#sm-b-tw').on(clickEvent, function() {
            window.open('http://twitter.com/StaRatingEPL', '_system'); return false;
          });
